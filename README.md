...
<ul>
  ...
  <li>Однако, HyperLogLog улучшает оценку за счет формулы для параметра оценки E</li>
  <li>Величина E рассчитывается на основе "карты" M[m], где m - число элементов карты (кратное степени 2). Число m рассчитывает на основе допустимой ошибки, например, если брать 6,5%, то количество 256 элементов, если меньше,то больше</li>
...
<p>
Если использовать "классический" подход с HashSet, то не смотря на то, что сложность всех операций будет O(1), "перегрев" памяти будет обеспечен.
</p>
<p>
...
<table>
  <tr>
    ...
    <th>Число уникальных элементов (Расчет через HashSet)</th>
    <th>Время работы HyperLogLog, мс</th>
    <th>Ошибка, %</th>
  </tr>
  <tr>
    <td>0</td>
